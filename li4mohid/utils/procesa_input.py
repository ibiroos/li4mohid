...
class THREDDS_parser:
    ...
    def parse_dates(self):
        ...
        dates = [datetime.strptime(re.findall(r'\d{10}',element.attrib['name'])[0],'%Y%m%d%H') for element in filtered if '.nc' in element.attrib['name']]
        print('ffffffffffffffffffffffffffff', dates[0]<dates[-1])
        if dates[0]<dates[-1]:
            dates.reverse()
        return dates
...
class outputReader:
    ...
    def get_layer(self):
        ...
        for fichero, fecha in dict(zip(self.ficheros, self.fechas)).items():
            ...
            if len(vars)==1:
                QgsMessageLog.logMessage("No data in file: %s date: %s " % (fichero, fecha), PLUGIN_NAME, level=Qgis.Info)
                continue
            else:
                QgsMessageLog.logMessage("Processing file: %s date: %s " % (fichero, fecha), PLUGIN_NAME, level=Qgis.Info)
